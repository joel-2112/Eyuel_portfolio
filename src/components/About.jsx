...
const About = () => {
  ...
  const services = [
    ...
    { title: "Mobile App Development", icon: <FaMobileAlt />, description: "Creating cross-platform mobile apps with Flutter." },
    { title: "Design", icon: <FaPaintBrush />, description: "Crafting intuitive and visually appealing UIs with Figma." },
  ];

  return (
    <div id="about" className="relative bg-gradient-to-br from-gray-50 via-indigo-50 to-white py-24 px-4 sm:px-6 lg:px-12 text-center overflow-hidden">
      ...
      >
        ...
        >
          I am Eyuel, a passionate software developer with expertise in web and mobile app development. I craft responsive,
          user-friendly applications with a focus on innovation, efficiency, and cutting-edge technology. Let’s collaborate
          ...
          >
            <Tilt options={{ max: 35, scale: 1.1, glare: true, "max-glare": 0.6 }}>
              <div className="relative bg-white rounded-xl p-6 h-72 flex flex-col items-center justify-center border-2 border-indigo-100 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                ...
                <h3 className="relative z-10 text-xl font-semibold text-gray-900 mb-3">{service.title}</h3>
                <p className="relative z-10 text-sm text-gray-600 text-center px-2">{service.description}</p>
              </div>
            </Tilt>
          </motion.div>
        ))}
